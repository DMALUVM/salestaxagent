...
# ── List existing reports (no createReport) ─────────────────
...
def list_reports(
    ...
    """List reports that already exist (Amazon-generated or previously requested).

    Settlement reports are generated by Amazon — we must NOT call createReport
    for them.  Instead we list existing DONE reports and download each one.

    Args:
        report_types: e.g. ["GET_V2_SETTLEMENT_REPORT_DATA_FLAT_FILE_V2"]
        created_since: ISO-8601 datetime string for createdSince filter
        page_size: max results per page (max 100)

    Returns:
        List of report dicts (each has reportId, processingStatus, etc.)
    """
    ...
    url = f"{BASE_URL}{REPORTS_PATH}/reports"
    ...
    while url:
        ...
        if resp.status_code != 200:
            raise SPAPIError(
                f"getReports failed ({resp.status_code}): {resp.text[:500]}"
            )
