...
| `AI_GATEWAY_API_KEY` | Vercel only | `/api/conversion-digest` (on read) and `/api/shopify-funnel/jev-triage` (cron warmup) | **Do not set on Mini.** Jev leak triage for the Conversion Digest. CoS/Dave add via Vercel → Project `dashboard` → Settings → Environment Variables / Secure Vault (Production + Preview). Never commit. Never paste into chat, repo, or Mini `.env`. Missing key → `hold_for_review` and `improvements: []` (fail closed, no LLM). |
