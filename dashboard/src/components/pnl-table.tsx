...
function feesBadge(basis: string, open: boolean) {
  if (open) {
    return (
...
export function PnlTable({
  ...
  const [grain, setGrain] = useState<PnlGrain>("day");
  const [lookback, setLookback] = useState<PnlLookback>("all");
  const [expandedKey, setExpandedKey] = useState<string | null>(null);
  const [expandedDay, setExpandedDay] = useState<string | null>(null);

  const periods = useMemo(
    () => buildPnlPeriods({ rows, grain, lookback, asOf }),
    [rows, grain, lookback, asOf],
  );
  const summary = useMemo(() => summarizePeriods(periods), [periods]);

  const storedMin = rows.reduce<string | null>((m, r) => (!m || r.date < m ? r.date : m), null);
  const storedMax = rows.reduce<string | null>((m, r) => (!m || r.date > m ? r.date : m), null);
  ...
  const lookbackHint = lookback === "all"
            ...
            {periods.length === 0 && (
              <TableRow>
                <TableCell colSpan={10} className="py-8 text-center text-sm text-muted-foreground">
                  No stored P&amp;L days in this lookback. Nightly sync keeps 90 days;
                  run <code>python -m src.main pnl-sync --days 90</code> to backfill.
                </TableCell>
              </TableRow>
            )}
            {periods.map((p) => {
              ...
              return (
                <Fragment key={p.key}>
                  ...
                  >
                    ...
                    <TableCell className="text-right tabular-nums text-muted-foreground">${fmtD(p.ads)}</TableCell>
                    ...
                    <TableCell>{feesBadge(p.feesBasis, p.open)}</TableCell>
                  </TableRow>
                  {open && (
