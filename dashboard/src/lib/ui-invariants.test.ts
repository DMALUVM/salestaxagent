...
describe("pallet planner ship view", () => {
  test("pallets page keeps only the ship plan, not cover-chrome", () => {
    ...
    assert.doesNotMatch(ship, /assorted \+ orange late September/);
...
describe("dashboard control labels match handlers", () => {
  ...
  test("reimbursements Sync is an enqueue, not a live Amazon pull", () => {
    const page = src("src/app/reimbursements/page.tsx");
