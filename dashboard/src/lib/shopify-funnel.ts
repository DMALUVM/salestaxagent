...
export const DEFINITIONS: Array<[string, string]> = [
  ...
  ["Triage", "Abandon-row severity is still a Mini stub. Jev leak triage runs on Vercel (/api/shopify-funnel/jev-triage, twice-daily cron) — Mini shopify-funnel-sync does not call an LLM. Missing AI_GATEWAY_API_KEY or a silent window → hold for review (fail closed)."],
