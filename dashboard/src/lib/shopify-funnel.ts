...
export const DEFINITIONS: Array<[string, string]> = [
  ...
  ["Triage", "Abandon-row severity is still a Mini stub. Jev leak triage runs on Vercel inside GET /api/conversion-digest (ensureFunnelJevTriage). Cron /api/shopify-funnel/jev-triage is warmup only. Mini shopify-funnel-sync stamps last_stats.jev (vercel_runtime / silent) and does not call an LLM. Missing AI_GATEWAY_API_KEY or a silent window → hold for review (no LLM). Digest improvements still emit from that locked day's Shopify leak plus GA4/GSC/Google Ads when material; Jev hold does not blank them. Empty only when nothing material. Never Meta in improvements (phase2.meta is locked-day facts only)."],
