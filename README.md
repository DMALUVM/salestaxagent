...
**SoldScope (optional Amazon intel):** set `SOLDSCOPE_API_TOKEN` on the Mac Mini `.env` (launchd) after creating a Personal Access Token in SoldScope → API Access. Do **not** commit the token. Daily `soldscope_daily_rt` (10:30 ET) stores reuse-only Rank Tracker phrase snapshots for existing hero groups (`as_of` = calendar day of a successful pull) and expands SoldScope phrases/v2 heatmap `r_YYYY-MM-DD` history (last 30 days, max 92) into extra as_of rows — only days SoldScope returns, never invented. Weekly `soldscope_weekly_sync` (Sunday 10:30 ET) stores hero history / ratings / capped search-volume when SoldScope has finished downloading the Amazon catalog (can take up to ~24h). Rank Tracker is read-only — 0 groups is a clean no-op; this agent never creates groups or phrases (`create_groups: false`). Keyword Research **reads** saved single-ASIN searches; a create is allowed only after history is non-empty and that hero has no RT phrases (max one POST per hero). Competitor reverse-ASIN KR (`soldscope-competitor-kr`) is cache-first weekly (Sunday 10:45 ET) — no full re-hit of all 30 unless missing/stale. First fill: `python -m src.main soldscope-competitor-kr --create-missing` (max 1 POST per ASIN, hard cap 5 POSTs/run, no wait-loop; HTTP 402 stops clean). Blake surface on `/ppc/gno` + GNO `competitor_kr_outliers.csv` is net-new unused Exact only (cap 5/family, 15 total) and requires competitor-on-SERP (`organic_asin` or `sponsored_asin` equals that row’s competitor ASIN) plus family-fit — rank>0 alone is not presence. Not a new research page. Vercel does not need the token (existing `/ppc`, `/ppc/gno`, and Amazon Ops surfaces read `soldscope_*` via the service role). Keyword outliers for Blake live on `/ppc/gno`. Apply `supabase/migration_soldscope.sql` once, then `supabase/migration_soldscope_rank_sfr.sql` and `supabase/migration_soldscope_rank_organic_asin.sql` (child ASIN holding each organic slot). Manual runs: `python -m src.main soldscope-daily-rt` and `python -m src.main soldscope-weekly-sync`.
...
| Daily 10:30 ET | SoldScope Rank Tracker | Reuse-only GET of existing hero RT groups → daily `as_of` columns (never create groups/phrases) |
| Sunday 10:30 ET | SoldScope weekly | Hero history + ratings + capped search-volume + KR (RT reuse-only fallback; 0 groups = no-op) |
| Sunday 10:45 ET | SoldScope competitor KR | Reuse saved searchType0 reverse-ASIN KR for 30 competitors (no create; first fill is CLI `--create-missing`) |
